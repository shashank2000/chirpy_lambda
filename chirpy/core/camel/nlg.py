...
class NLGHelper:
    ...
    def generate(self, context):
        assert hasattr(
            context.supernode.nlg_helpers, self.name
        ), f"Function not found: {self.name} (available: {dir(context.supernode.nlg_helpers).filter(lambda x: not x.startswith('_'))})"
...
class DatabaseLookup(NLGNode):
    ...
    def generate(self, context):
        result = lookup(self.database_name.generate(context), self.key.generate(context))
