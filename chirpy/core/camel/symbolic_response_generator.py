...
class SymbolicResponseGenerator(ResponseGenerator):
	...
	def get_response(self, state) -> ResponseGeneratorResult:
		...
		context = self.get_context(state, utterance)
		supernode = self.get_takeover_or_current_supernode(state, python_context, contexts)
		context.supernode = supernode
		context.utilities = self.get_utilities(supernode)		
		...
		while True:
			...
			logging.primary_info(f"Flags for supernode {supernode} are: {flags}")
