...
CONFIRM_SAYING_PHRASES = ["Yes, that's what I said", "Yeah, that's right", "Yep", "Yeah", "That's right"]
CORRECT_SAYING_PHRASES = ["No, I said", "Actually, I said", "Oh, sorry, I meant", "Oops, I actually said"]
THANK_INQUIRY_PHRASES = ["Thanks for asking!", "I'm glad you asked."]
...
GOT_FACT_CHECKED_PHRASES = ["Oops,", "Hmm, I think you might be right"]
...
BOT_REPEAT_PHRASES = (
    CONFIRM_SAYING_PHRASES + CORRECT_SAYING_PHRASES + REPETITION_APOLOGY + response_lists.CLARIFICATION_COMPLAINT_RESPONSE
)
...
def user_asked_personal_question(state_manager):
    ...
    if pq_slots:
        logger.primary_info(f"Detected personal question slots: {pq_slots}")
    return pq_slots
...
def is_game_or_music_request(state):
    ...
    if state.entity_tracker.cur_entity_initiated_by_user_this_turn(state):
        for ent_group in [
            ENTITY_GROUPS_FOR_CLASSIFICATION.musician,
            ENTITY_GROUPS_FOR_CLASSIFICATION.musical_group,
            ENTITY_GROUPS_FOR_CLASSIFICATION.musical_work,
        ]:
            if ent_group.matches(cur_entity):
                found_musical_entity = True
    return did_not_ask_user_activity and (
        (request_play_slots is not None and found_musical_entity)
        or (request_play_slots is not None and not_request_play_slots is None)
    )
...
def user_said_anything(state_manager):
    ...
    if any(previous_bot_utterance.endswith(i) for i in triggers):
        if any(i in utterance for i in nothing_replies):
            return True
...
def user_got_cutoff(state_manager):
    ...
    return cutoff_slot or (
        nav_intent_output.pos_intent and nav_intent_output.pos_topic_is_hesitate and "depends on" not in utterance
    )
