...
def get_sections(
    entity: WikiEntity,
    suggested_sections: List[WikiSection],
    discussed_sections: List[WikiSection],
    last_discussed_section: WikiSection,
):
    sections = wiki_utils.get_wiki_sections(entity.name)
    valid_sections = filter_and_log(
        lambda section: section not in suggested_sections,
        sections,
        "Wiki Section",
        reason_for_filtering="these sections were suggested",
    )
    valid_sections = filter_and_log(
        lambda section: section not in discussed_sections,
        valid_sections,
        "Wiki Section",
        reason_for_filtering="these sections were discussed",
    )
    ...
    if last_discussed_section is not None:
        ...
        if last_discussed_section.level() == 2:
            ...
            parent_sections = list(filter(lambda section: section.title == parent_section_title, sections))
            if len(parent_sections) > 0:
                ...
                siblings = list(
                    filter(
                        lambda section: section.ancestor_titles and section.ancestor_titles[-1] == parent_section_title, sections
                    )
                )
                ...
                logger.info(
                    f"One more sibling of {parent_section_title} has already been discussed. Not suggesting more sibling subsections."
                )
    ...
    if first_level_sections:
        logger.primary_info(f"Choosing from {[s.title for s in first_level_sections]} 1st level sections")
    ...
    filtered_first_level_section_titles = filter_and_log(
        lambda f_section_title: len(
            [
                s
                for s in discussed_sections
                if (s.level() >= 2 and s.ancestor_titles[-1] == f_section_title) or s.title == f_section_title
            ]
        )
        <= 3,
        first_level_section_titles,
    ...
    if filtered_first_level_section_titles:
        return {
            "parent_section": None,
            "sections": filtered_first_level_section_titles,
            "repeat": last_discussed_section is not None,
        }
...
def get_section_choices(entity: WikiEntity, chosen_sections: List[str], parent_section: Optional[WikiSection]):
    ...
    section_choices, entitys_section_choices = construct_entity_section_choices(
        entity.talkable_name, chosen_sections_titles, "or" if parent_section else "and"
    )
    return {
        "section_choices": wiki_utils.clean_wiki_text(section_choices),
        "entitys_section_choices": wiki_utils.clean_wiki_text(entitys_section_choices),
    }
...
def get_selected_section(
    entity: WikiEntity,
    suggested_sections: List[WikiSection],
    chosen_sections: List[WikiSection],
    response_text: str,
    GlobalFlag__YES: bool,
):
    """
    ...
    for option in prompted_options:
        ...
        if any(
            editdistance.eval(u_token, eu_token) < 2 for u_token in utterance.split(" ") for eu_token in option.lower().split(" ")
        ):
