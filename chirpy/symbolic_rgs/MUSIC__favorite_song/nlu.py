...
from chirpy.response_generators.music.music_helpers import tags
...
# def get_song_meta(song_name, singer_name=None):
#     if ' by ' in song_name:
#         song_name, singer_name = song_name.split(' by ')
#     musicbrainz = MusicBrainzInterface()
#     metadata = musicbrainz.get_song_meta(song_name, singer_name)
#     if metadata:
...
def get_flags(context):
    ...
    if song_ent:
        ...
        # metadata = get_song_meta(song_str, singer_ent.talkable_name if singer_ent else None)
    else:
        ...
        if song_slots is not None and 'favorite' in song_slots:
            ...
            # metadata = get_song_meta(song_str, None)

    # if metadata:
    #     ADD_NLU_FLAG('MUSIC__song_metadata_exists')
    #     ADD_NLU_FLAG('MUSIC__song_metadata_song', metadata["song"])
    #     ADD_NLU_FLAG('MUSIC__song_metadata_artist', metadata["artist"])
    #     ADD_NLU_FLAG('MUSIC__song_metadata_year', metadata["year"])
    #     ADD_NLU_FLAG('MUSIC__singer_of_fav_song', metadata["artist"])
    # else:
    #     ADD_NLU_FLAG('MUSIC__singer_of_fav_song', None)
    ADD_NLU_FLAG('MUSIC__singer_of_fav_song', "Taylor Swift")  # TODO: get singer using musicbrainz
