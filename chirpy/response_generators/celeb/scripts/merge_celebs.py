...
def merge_opinions():
    celeb_opinion_file = open("celeb_opinions_indiv.jsonl").readlines()
    merged_celeb_ops = {}
    for l in celeb_opinion_file:
        ld = json.loads(l)
        merged_celeb_ops.update({
            ld["celeb"]: {
                "opinion": ld["opinion"]
            }
        })
...
if __name__ == "__main__":
    ...
    # new_celeb_ops = merge_opinions()
    # orig_celeb_ops = json.load(open("individual_celeb_opinions.json"))
    # orig_celeb_ops.update(new_celeb_ops)
