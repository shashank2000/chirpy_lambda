...
def is_gratitude_response(state_manager, utterance):
    ...
    """
    return state_manager.current_state.corenlp['sentiment'] >= Sentiment.NEUTRAL and \
...
def is_noncommittal_response(state_manager, utterance):
    ...
    top_da = state_manager.current_state.dialogact['top_1']
    return top_da in ['back-channeling'] or len(utterance.split()) <= 5


PERSONAL_ISSUE_THRESHOLD = 0.7
def is_personal_issue(state_manager, utterance):
    sentiment = state_manager.current_state.corenlp['sentiment']
...
def is_continued_sharing(state_manager, utterance):
    ...
    personal_disclosure = state_manager.current_state.dialogact['personal_issue_score'] >= NEUTRAL_SHARING_THRESHOLD
    ...
    if state_manager.current_state.question['is_question'] or change_topic:
        return False
