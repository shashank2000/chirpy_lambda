...
from chirpy.response_generators.wiki2.response_templates.response_components import (
    ...
    COMMISERATION_ACKNOWLEDGEMENTS,
)
...
def user_is_confused(context: Context, utterance: str):
    return (
        ClarificationQuestionTemplate().execute(utterance)
        and not context.state_manager.current_state.navigational_intent.pos_intent
...
def is_high_initiative(context: Context, utterance: str):
    ...
    return not (
        (len(utterance) <= 4 and not corenlp_output["nouns"] and not corenlp_output["proper_nouns"]) or len(utterance) <= 3
    )
...
def is_appreciative(context: Context, utterance: str):
    return (
        context.state_manager.current_state.dialogact["top_1"] == "appreciation"
        or AppreciativeTemplate().execute(utterance) is not None
    )
...
FIRST_PERSON_WORDS = {"i", "i'd", "i've", "i'll", "i'm", "me", "my", "myself", "mine"}
...
def is_opinion(context: Context, utterance: str):
    return (
        utterance.startswith("because")
        or "opinion"
        in (context.state_manager.current_state.dialogact["top_1"], context.state_manager.current_state.dialogact["top_2"])
        or any([utterance.startswith(x) for x in ["i believe", "i think", "i feel"]])
    )
...
def add_flags(context: Context, add_nlu_flag) -> Set[str]:
    ...
    if user_is_confused(context, utterance):
        add_nlu_flag("GlobalFlag__CONFUSED")
    if is_neg_sentiment(context, utterance):
        add_nlu_flag("GlobalFlag__NEG_SENTIMENT")
    if is_pos_sentiment(context, utterance):
        add_nlu_flag("GlobalFlag__POS_SENTIMENT")
    if is_neutral_sentiment(context, utterance):
        add_nlu_flag("GlobalFlag__NEUTRAL_SENTIMENT")
    if is_opinion(context, utterance):
        add_nlu_flag("GlobalFlag__OPINION")
    if is_appreciative(context, utterance):
        add_nlu_flag("GlobalFlag__APPRECIATIVE")
    ...
    if is_personal_disclosure(context, utterance):
        add_nlu_flag("GlobalFlag__PERSONAL_DISCLOSURE")
    if user_disagrees(context, utterance):
        ...
        if user_agrees(context, utterance):
            add_nlu_flag("GlobalFlag__AGREEMENT")
    # if is_no_to_sections(self, utterance): response_types.add(ResponseType.NO)
    if starts_with_what(context, utterance):
        add_nlu_flag("GlobalFlag__STARTS_WITH_WHAT")
