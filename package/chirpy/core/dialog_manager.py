...
from chirpy.response_generators.closing_confirmation.closing_confirmation_response_generator import (
    CLOSING_CONFIRMATION_STOP,
)
...
class DialogManager:
    ...
    def __init__(
        ...
        from chirpy.core.response_generator.symbolic_response_generator import (
    ...
    @measure
    def execute_turn(self, kwargs: Dict[Any, Any]) -> (str, str, bool):
        """
        ...
        """
        logger.warning(f"Kwargs (dialogue_manager) are {kwargs}")
        ...
        response = self.response_generator.get_response(state, self.state_manager.current_state.text, kwargs=kwargs)
    ...
    def update_entity_tracker_state(self):
        ...
        if update_entity_result.update:
            logger.primary_info(
                f"Ran last_active_rg={last_active_rg}'s get_entity() function. It returned "
                f"{update_entity_result}, so using this to update the entity tracker state",
                extra={"color_msg_by_component": last_active_rg},
            )
            self.state_manager.current_state.entity_tracker.update_from_rg(
                update_entity_result, last_active_rg, self.state_manager.current_state
            )
        else:
