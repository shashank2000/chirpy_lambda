...
try:
    from chirpy.core.logging_utils import (
        setup_logger,
        update_logger,
        get_bluejay_logger_settings,
    )
...
logger = logging.getLogger("chirpylogger")
apology_string = (
    "Sorry, I'm having a really hard time right now. "
...
class StateTable:
    ...
    def persist(self, state: Dict):
        ...
        try:
            ...
            logger.error(
                "Exception when persisting state to table" + self.table_name,
                exc_info=True,
            )
            return False
...
remote_url_config = {
    "corenlp": {"url": "http://localhost:4080"},
    "dialogact": {"url": "http://localhost:4081"},
    "g2p": {"url": "http://localhost:4082"},
    "gpt2ed": {"url": "http://localhost:4083"},
    "question": {"url": "http://localhost:4084"},
    "convpara": {"url": "http://localhost:4085"},
    "entitylinker": {"url": "http://localhost:4086"},
    "blenderbot": {"url": "http://localhost:4087"},
    "responseranker": {"url": "http://localhost:4088"},
    "stanfordnlp": {"url": "http://localhost:4089"},
    "infiller": {"url": "http://localhost:4090"},
}
